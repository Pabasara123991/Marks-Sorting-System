...
if User_option == 1:
    ...
    Retriever_list= []
    Excluded_list= []
    Credit = open("Final_outcome.txt","w")
    ...
    while True:
        ...
        if total != 120:        #check whether credit total equal to 120
            ...
            Progress_count += 1
            Credit.write(f'Progress - {Pass},{Defer},{Fail} \n')
            
        elif Pass == 100 and Defer <= 20 and Fail <= 20:
            print("Module trailer")
            Trailer_list.append([Pass,Defer,Fail])
            Trailer_count += 1
            Credit.write(f'Module trailer- {Pass},{Defer},{Fail} \n')
            
        elif Pass <=80 and Defer <= 80 and Fail <= 60:
            print("Module retriever")
            Retriever_list.append([Pass,Defer,Fail])
            Retriever_count += 1
            Credit.write(f'Module retriever - {Pass},{Defer},{Fail} \n')
            
        else:
            print("Excluded")
            Excluded_list.append([Pass,Defer,Fail])
            Excluded_count += 1
            Credit.write(f'Excluded - {Pass},{Defer},{Fail} \n')
